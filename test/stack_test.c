...
bool move_equal(move move1, move move2) {
    return move1->i == move2->i && move1->j == move2->j && move1->s == move2->s;
}
...
bool test_stack_is_empty(void) { 
    ...
    if(!stack_is_empty(test)) { return false; }
...
bool test_stack_push_head(void) { 
    ...
    if(!move_equal(move1, stack_peek_head(test))) { return false; }
    test = stack_push_head(test, move2);
    if(!move_equal(move2, stack_peek_head(test))) { return false; }
